...
def _doctor_command(
    *,
    ...
    lines = [
        ...
        'echo "has_sshd_bin=$( [ -x /usr/sbin/sshd ] && echo 1 || echo 0 )"',
        'echo "has_dropbear_bin=$( [ -x /usr/sbin/dropbear ] && echo 1 || echo 0 )"',
        'echo "ps_sshd=$(ps -ef | grep -c \\"[s]shd -p $SSH_PORT\\" 2>/dev/null || true)"',
        'echo "ps_dropbear=$(ps -ef | grep -c \\"[d]ropbear.*-p.*$SSH_PORT\\" 2>/dev/null || true)"',
        'echo "ps_rtunnel=$(ps -ef | grep -Ec \\"[r]tunnel .*([[:space:]]|:)$PORT([[:space:]]|$)\\" 2>/dev/null || true)"',
