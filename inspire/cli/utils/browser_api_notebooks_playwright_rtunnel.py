    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if dropbear_deb_dir:
                ...
                if not setup_script:
                    raise ValueError(
                        "INSPIRE_SETUP_SCRIPT environment variable is required when using "
                        "dropbear. It should point to a script that installs dropbear and starts "
                        "rtunnel."
                    )
                ...
                cmd_lines.extend(
                    [
                        ...
                        (
                            'if [ -n "${RTUNNEL_BIN_PATH:-}" ] && [ -x "$RTUNNEL_BIN_PATH" ]; '
                            'then cp "$RTUNNEL_BIN_PATH" /tmp/rtunnel && chmod +x /tmp/rtunnel; '
                            "fi"
                        ),
