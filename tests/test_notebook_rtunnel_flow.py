...
from inspire.platform.web.browser_api.notebooks.playwright.rtunnel import flow as flow_module
...
def test_ensure_proxy_readiness_uses_fallback_when_primary_fails(
    ...
    assert calls == [primary_url, fallback_url]
    assert len(diagnostics) == 1
    assert diagnostics[0].startswith("primary=")
...
def test_ensure_proxy_readiness_does_not_raise_when_both_probes_fail(
    ...
    assert diagnostics[0].startswith("primary=")
    assert diagnostics[1].startswith("fallback=")
