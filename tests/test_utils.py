...
class TestProxyCommand:
    ...
    def test_install_all_ssh_configs_replaces_legacy_block(
        ...
        """Managed install should replace legacy Inspire entries and preserve user hosts."""
        ...
        config = TunnelConfig(config_dir=tmp_path)
        config.add_bridge(BridgeProfile(name="cpu", proxy_url="https://new.example/ws/cpu/proxy/31337/"))
        config.add_bridge(BridgeProfile(name="gpu", proxy_url="https://new.example/ws/gpu/proxy/31337/"))
        monkeypatch.setattr("inspire.bridge.tunnel.ssh._ensure_rtunnel_binary", lambda _: tmp_path / "rtunnel")
    ...
    def test_has_installed_ssh_config_detects_managed_block(
        ...
        """Managed block marker should be enough to detect installed config."""
        monkeypatch.setenv("HOME", str(tmp_path))
        ssh_dir = tmp_path / ".ssh"
        ssh_dir.mkdir(parents=True, exist_ok=True)
        ssh_config_path = ssh_dir / "config"
    ...
    def test_install_all_ssh_configs_replaces_unmanaged_generated_single_host_entries(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """Managed install should replace older single-host Inspire entries too."""
        monkeypatch.setenv("HOME", str(tmp_path))
        ssh_dir = tmp_path / ".ssh"
        ssh_dir.mkdir(parents=True, exist_ok=True)
        ssh_config_path = ssh_dir / "config"
        ...
        assert "Host personal" in content
        assert "old.example" not in content
        assert content.count("Host cpu") == 1
        assert "new.example" in content

    def test_get_ssh_command_args_uses_cleanup_wrapper(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """Runtime SSH wrapper should clean stale rtunnel listeners before bootstrap."""
        config = TunnelConfig(config_dir=tmp_path)
